...
function useToggle({
  ...
  React.useEffect(() => {
    ...
    warning(
      ...
      `An \`on\` prop was provided to useToggle without an \`onChange\`handler. This will render a read-only toggle. If you want it to be a mutable, use \`onChange\` or \`readOnly\`. `,
    )
    // Previous implementation of warning
    /*
  ...
  function dispatchWithOnChange(action) {
    // if the state is being controlled, then we don't want to rerender the internally managed state
    if (!onIsControlled) {
      dispatch(action)
    }
    // const newSate = reducer(state, action)
    // onChange is optional; suggestied state change; action
    onChange?.(reducer({...state, on}, action), action)
  }
